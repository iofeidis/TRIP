...
logFile = paste0(tmp_path,"/log_files/log_file_",trunc(as.numeric(Sys.time())),".txt")
...
save_tables_individually = T #libraries.R TRUE

if (save_tables_individually | save_lists_for_bookmark){
  #output folder 
  output_folder=paste0(getwd(),"/output/output_tables ",trunc(as.numeric(Sys.time())))
